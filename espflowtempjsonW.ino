...
Serial.print(flow1count/450.0);  // adjust 450 to match the alue of your flow metre 
...
dta= (dta+dt); 
 ...
 printData(inputb);
  printData(outputb);
  t3 = (sensors.getTempC(inputb)+0.00);  //add error correction
...
dt2a=(dt2a+dt2);
  ...
  if (now - lastMsg > 5000) {   // 5 second sample
    lastMsg = now; 
    fa =((flowacount/450.0)/5);  //flow rate calulated at l/s
    fb=((flowbcount/450.0)/5);
    dta=(dta/cnt);
    dt2a=(dt2a/cnt);

    w=(((((flowacount/450.0)*1000)*dta)*4.188)/5);     // adjust last to match inteval of sampling sec
